...
export const staleOdrLinkRule: Rule = {
  ...
  evaluate(evidence: ScanEvidence): Finding | null {
    ...
    const explanation =
      ...
      'в заблуждение. Вместо неё для споров с потребителями в Латвии следует направлять ' +
      'в «Patērētāju tiesību aizsardzības centrs (PTAC), Patērētāju strīdu risināšanas ' +
      'komisija» (Центр защиты прав потребителей Латвии, Комиссия по разрешению ' +
      'потребительских споров) — устаревшую ссылку стоит убрать или заменить этой информацией.';

    const remediation = [
      'Удалите ссылку на платформу ODR (ec.europa.eu/consumers/odr) — она больше не действует.',
      'Укажите, куда клиент может обратиться со спором в Латвии: «Patērētāju tiesību ' +
        'aizsardzības centrs (PTAC), Patērētāju strīdu risināšanas komisija» (Центр защиты ' +
        'прав потребителей Латвии, Комиссия по разрешению потребительских споров).',
