...
- **Бэкап на GitHub:** репозиторий `AndrewKuchik/violation-scanner`, ветка `master`. Пушить после значимых изменений (`git push origin master`). **ПУБЛИЧНЫЙ** (стал публичным намеренно — см. ниже).
- **Деплой:** задеплоен на Vercel (бесплатный публичный адрес `*.vercel.app`), продакшн-ветка `master` — **push → авто-пересборка живого сайта за ~2–3 мин**, отдельных действий не нужно. Облачный браузер: `@sparticuz/chromium` (см. `../deploy-vercel.md`). Env на Vercel: `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1`. Локальный режим сохранён.
- **Почему репозиторий публичный:** проект на Vercel живёт в команде «FLOPPY ENTERPRISES», а коммиты подписаны почтой `admin@bayareawindowpros.com` (личность `bayareawindowpros`), которая не участник этой команды → на бесплатном плане Vercel деплой приватного репо блокировался («Failed deployment… not a member of the team», просил Pro). Решили без оплаты — сделали репозиторий публичным (`gh repo edit … --visibility public`); секретов в коде нет (ключи только в env Vercel, `.env*` в .gitignore). Альтернатива, если снова понадобится приватность: перенести проект в личный аккаунт Vercel (Hobby) вместо команды.
